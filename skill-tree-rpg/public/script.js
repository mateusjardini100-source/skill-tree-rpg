...
submitTreePassword.onclick = async () => {
    ...
    await loadTreePerks(pendingTreeId);
...
function drawSkillTree() {
    ...
    for (const perk of currentPerks) {
        ctx.beginPath();
        ctx.arc(perk.pos_x, perk.pos_y, 20, 0, 2 * Math.PI);
        ctx.fillStyle = perk.is_purchased ? '#ffd966' : '#2c3e50';
        ctx.fill();
        ctx.strokeStyle = '#d4af37';
        ctx.lineWidth = 2;
        ctx.stroke();
...
skillCanvas.addEventListener('click', async (e) => {
    ...
    const clickedPerk = currentPerks.find(p => Math.hypot(p.pos_x - mouseX, p.pos_y - mouseY) < 20);
...
loginAdminBtn.onclick = async () => {
    if (adminPassword.value === 'Admin') {  // senha fixa
        isAdminMode = true;
